...
public class BlogFileService : IBlogFileService
{
    ...
    public async Task UpdateBlogPostAsync(BlogPost blogPost, string originalPostLink,
    ...
    public async Task AddBlogPostAccessAsync(string postLink, DateTime date, int count,
        CancellationToken cancellationToken = default)
    {
        var blogPost = await GetBlogPostAsync(postLink, cancellationToken);
        if (blogPost == null)
