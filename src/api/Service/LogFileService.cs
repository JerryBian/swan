...
namespace Laobian.Api.Service
{
    public class LogFileService : ILogFileService
    {
        ...
        public async Task AddLogAsync(LaobianLog log, CancellationToken cancellationToken = default)
        {
            ...
            await _logFileRepository.AppendLineAsync(Path.Combine(site.ToString().ToLowerInvariant(), log.TimeStamp.Year.ToString("D4"), $"{log.TimeStamp.ToDate()}.log"),  JsonUtil.Serialize(log), cancellationToken);
        }
    }
}
