...
namespace Laobian.Admin.HttpService
{
    public class BlogHttpService
    {
        ...
        public BlogHttpService(HttpClient httpClient, ILogger<BlogHttpService> logger, IOptions<AdminConfig> config)
        {
            _logger = logger;
        ...
        //    await using var stream = await response.Content.ReadAsStreamAsync();
        //    return await JsonHelper.DeserializeAsync<BlogComment>(stream);
        //}
