...
namespace Laobian.Share
{
    public class HtmlHeadBuilder
    {
        ...
        private readonly CommonOption _option;

        public HtmlHeadBuilder(CommonOption option, HtmlHeadBuildOption buildOption)
        {
            _option = option;
        ...
        public string Build()
        {
            ...
            if (!string.IsNullOrEmpty(_buildOption.Keywords))
            {
                sb.AppendLine($"<meta name=\"keywords\" content=\"{_buildOption.Keywords}\"/>");
            }
            ...
            {
                title = $"{_buildOption.Title} &minus; " + title;
            }
